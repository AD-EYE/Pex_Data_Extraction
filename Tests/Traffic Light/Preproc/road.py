...
class StraightRoad(Road):
    '''
    This a representation of the straight road in Prescan.

    :param id: Unique id.
    :type id: String

    :param x0: The x coordinate of the starting point of the straight road
    :type x0: Float

    :param y0: The y coordinate of the starting point of the straight road
    :type y0: Float

    :param h: Global heading of the straight road at its starting point
    :type h: Float

    :param l: Lenght of the straight road
    :type l: Float
    ...
    def __init__(self, id, x0, y0, h, l, lw, nbr_of_lanes, lanes_in_x_dir, SpeedL, RefS):
        ...
        # Lanes, Center and Edges

        self.c.append(Straight( x0, y0, h, l))         #Need FiXing

        self.e1.append(Straight( x0 - (nbr_of_lanes/2)*lw * np.cos(h + np.pi / 2),
                                y0 - (nbr_of_lanes/2) * lw * np.sin(h + np.pi / 2), h, l))    #e1 is a tab of path object for now

        self.e2.append(Straight( x0 + (nbr_of_lanes/2)*lw * np.cos(h + np.pi / 2),
                                y0 + lw * (nbr_of_lanes/2) * np.sin(h + np.pi / 2), h, l))   #Same
